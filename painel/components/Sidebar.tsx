  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed top-4 left-4 z-50 md:hidden p-3 rounded-xl bg-gradient-to-r from-rose-500 to-pink-600 text-white shadow-lg shadow-rose-300/40"
      >
      ...
      <aside
        className={`
          fixed top-0 left-0 h-screen w-[270px] z-40
      ...
      >
        ...
        <div className="px-7 pt-8 pb-6">
          <div className="flex items-center gap-3.5">
            <div className="flex items-center justify-center w-11 h-11 rounded-xl bg-gradient-to-br from-rose-400 to-pink-500 shadow-lg shadow-rose-500/25">
            ...
            <div className="min-w-0">
              <h1 className="text-[15px] font-bold text-white truncate leading-tight">{businessName}</h1>
              <p className="text-[11px] text-gray-500 font-medium mt-0.5">Painel Administrativo</p>
            </div>
          </div>
        </div>
        ...
        <div className="px-7 pt-2 pb-3">
          <span className="text-[10px] font-semibold text-gray-600 uppercase tracking-[0.12em]">Menu</span>
        </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                onClick={() => setMobileOpen(false)}
                className={`
                  group flex items-center gap-3 px-4 py-3 rounded-xl text-[13px] font-medium
                  transition-all duration-200 relative
                  ${isActive
                    ? 'bg-gradient-to-r from-rose-500/15 to-pink-500/10 text-rose-400'
                    : 'text-gray-400 hover:text-gray-200 hover:bg-white/[0.04]'
                  }
                `}
              >
                {isActive && (
                  <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-7 rounded-r-full bg-rose-400" />
                )}
                <span className={`transition-colors duration-200 ${isActive ? 'text-rose-400' : 'text-gray-500 group-hover:text-gray-300'}`}>
                  {item.icon}
                </span>
        ...
        <div className="px-7 py-5 space-y-3">
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white/[0.03]">
            ...
            <div>
              <p className="text-[11px] font-semibold text-gray-300">WhatsApp Bot</p>
